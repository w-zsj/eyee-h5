<template>
  <ul class="stack">
    <li
      ...
      :key="index"
      :style="[transformIndex(index)?transformIndex(index):{},transform(index)?transform(index):{}]"
...
export default {
  ...
  computed: {
    ...
    // 划出宽度比例
    offsetWidthRatio() {
      let width = this.$el && this.$el.offsetWidth;
      let offsetWidth = width - Math.abs(this.temporaryData.poswidth);
  ...
  mounted() {
    // console.log("this.$el", document.querySelector(".stack"));
    // 绑定事件
    this.$on("next", () => {
      this.next();
    });
  ...
  methods: {
    ...
    onTransitionEnd(index) {
      ...
      if (this.temporaryData.swipe && index === lastPage) {
        ...
        // console.log(this.temporaryData.currentPage, "eq");
        document.body.classList.remove("lock");
        this.$emit("currentPage", this.temporaryData.currentPage);
      }
    },
    prev() {
      this.temporaryData.tracking = false;
    ...
    transform(index) {
      ...
      if (this.inStack(index, currentPage) && this.temporaryData.prefixes) {
        let perIndex =
          index - currentPage > 0
            ? index - currentPage
      ...
      } else if (index === lastPage && this.temporaryData.prefixes) {
        style["transform"] =
          "translate3D(" +
