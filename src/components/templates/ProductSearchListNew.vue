<template>
  <div class="shop_product" :class="{ disShop_fix: styleType == 2 }">
    <div
      :class="{ fix_top: isShowNavi, discount_fix: styleType == 2 }"
      :style="insq ? 'top:0;' : ''"
    >
      <!-- y!=1&& -->
      <div
        class="brand_logo"
        v-if="brandInfo && showInfo && brandInfo.logourl && chadiaoData != 1"
      >
      ...
      <div class="tab_sorttype clearfix">
        ...
        <label
          :class="{ clicked: sortPrice[0].selected }"
          @click="selectPrice(sortPrice[0])"
        >
          <span
            class="font-pf"
            :class="{
              'arrow_span_down': sortPrice[0].updown,
              'e-down': sortPrice[0].updown,
              'arrow_span_up': !sortPrice[0].updown,
              'e-up': !sortPrice[0].updown,
            }"
  ...
  data() {
    return {
      ...
      sorttypes: [
        ...
        { sorttype: 2, name: "行情", updown: false, selected: false },
...
.filter_btn:after {
  ...
  width: 3.2vw;
  height: 3.2vw;
  background: url("../../../static/img/icon_screen_small.png") no-repeat;
...
.shop_product .tab_sorttype label .arrow_span_up:after {
  content: "";
  display: inline-block;
  position: absolute;
...
.shop_product .tab_sorttype label .arrow_span_down:after {
  content: "";
  display: inline-block;
  position: absolute;
  border-left: 4px solid transparent;
...
.shop_product .tab_sorttype > label.clicked span.e-down:after {
  /*border-top: none;
    border-bottom: 6px solid #000;*/
  /*bottom: 4.5px;*/
...
.shop_product .shop_prodlist .prod-item:nth-child(odd) {
  margin-right: 2.67vw;
}
